...
This module targets `Pulp #1991`_.
Signature check will be done when importing/associating packages
to RPM repositories. The importing or associating of packages would fail
if it doesn't meet the conditions set in repo importer configurations.

There will be 2 optional parameters in repo importer configurations:
1. allow_unsigned (boolean) : whether units with no signature is allowed
2. allow_keys (list):  units have to been signed with keys in allow_keys list
are allowed.
...
class _BaseAssoicateRPMTestCase(utils.BaseAPITestCase):
    ...
    @classmethod
    def setUpClass(cls, rpm_url, unit_type):
    ...
    def _associate_pkg_and_validate(self, repo_type, expect_success):
        ...
        if expect_success:
            self.expect_success(task)
        else:
            self.expect_fail(task)
